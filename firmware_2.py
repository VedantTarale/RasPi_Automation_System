...
def pump_water():
    ...
    global temperature
    global pressure
    global fill_status # is 1 when we do not need to fill water, is 0 when we have to fill water

    final_val = 220 #to be modified later
    pump_val = 140 #to be modified later
    margin = 5 #to be modified later
    GPIO.output(pump,GPIO.LOW)
    print("water value: ", water_reading)

    #setting fill_status values
    if (fill_status == 0) and (water_reading > final_val):
        fill_status = 1
    elif (fill_status == 1) and (water_reading < pump_val):
        fill_status = 0
    
    #deciding whether to pump or not based on current fill state and current water level
    if (water_reading < pump_val) or (((final_val - water_reading) > margin) and (fill_status == 0)):
    ...
    elif (water_reading > final_val) or ((water_reading > pump_val - margin) and (fill_status == 1)):
        GPIO.output(pump,GPIO.LOW)
